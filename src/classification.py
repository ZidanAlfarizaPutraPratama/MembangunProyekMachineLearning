...
def preprocess_data(data):
    ...
    data['Job_Role'] = data['Job_Role'].fillna(data['Job_Role'].mode()[0])
...
def analyze_clusters(X_transformed, data, best_n_clusters):
    kmeans = KMeans(n_clusters=best_n_clusters, random_state=42)
    data['Cluster'] = kmeans.fit_predict(X_transformed)  # Gunakan X_transformed untuk clustering
...
def classify_data(X_transformed, y):
...
if __name__ == "__main__":
    data = load_data('../data/Impact_of_Remote_Work_on_Mental_Health.csv')
    X_transformed, processed_data = preprocess_data(data)
    best_n_clusters = perform_clustering(X_transformed)
    analyze_clusters(X_transformed, processed_data, best_n_clusters)  # Perbaiki pemanggilan fungsi
    classify_data(X_transformed, processed_data['Cluster'])
